...
import { isAllStatusGivenAccounts, isNotGivenType, ValidationStringToFuncPointer } from './business.logic.validations.js';
...
import { IChangeStatus, IChangeStatusAccounts, IFamilyAccountModel, IModifyFamilyAccount } from '../../types/models.types.js';

function runTransferValidationFunctions(validationTranserName:string, source : IAccountDTO, 
  destination: IAccountDTO, amount:number):void{
...
export async function validateTransferB2B(sourceId: number, destinationId: number, 
  ...
  runTransferValidationFunctions(LogicValidationPerRoute.validateTransferB2B, source, destination, amount);
...
export async function validateTransferB2BFX(
  ...
  const source = await builderSQL.getBusinessAccountById(sourceId);
  const destination = await builderSQL.getBusinessAccountById(destinationId);
  runTransferValidationFunctions(LogicValidationPerRoute.validateTransferB2BFX, source, destination, amount);
  return { source, destination };
}
...
export async function validateTransferB2I(
  ...
  runTransferValidationFunctions(LogicValidationPerRoute.validateTransferB2I, source, destination, amount);
  return { source, destination };
}
...
export async function validateTransferF2B(
  ...
  const destination = await builderSQL.getBusinessAccountById(destinationId);
  runTransferValidationFunctions(LogicValidationPerRoute.validateTransferF2B, source, destination, amount);
  return { source, destination };
}
...
export async function validateActivateDeactivateAccounts(input : IChangeStatus) : Promise<IChangeStatusAccounts>{
  ...
  if (errors.length > 0) throw new ValidationException(`one of the accounts isnt in correct status to perform ${input.action}`);
  //check no accounts of type family
  errors.push(...isNotGivenType([...indivDTOS, ...businessDTOS], 'family'));
  if (errors.length > 0) throw new ValidationException('one of the accounts is type of family where it shouldnt be');

  const retObj = { individuals: indivDTOS,
...
export async function additionIndividualsToFamily(family_accout_id : number, model : IModifyFamilyAccount) : Promise<IModifyFamilyAccount>  {
  //logic validations
  /**
  ...
  individuals.reduce( (acc, curr, i) => {  
    if (curr.individual_account_id !== null &&   //all exist
      curr.status_id === 1 &&                    //all active
      curr.type_name === 'individual' &&        //all type individual
      curr.currency === family.currency){ //same currency as family
      tuples.push([ids[i], contributions[i]]);
    }
    return acc;
  }, tuples);
...
// export async function removalIndividualsFromFamily(family_accout_id : number, model : IModifyFamilyAccount) : Promise<IModifyFamilyAccount>  {
//   /**
